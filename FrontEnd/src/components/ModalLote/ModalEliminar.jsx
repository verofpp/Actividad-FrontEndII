...
import { eliminarLote } from "../../services/LotesService";
...
const ModalEliminarLote = ({
  ...
  useEffect(() => {
    if (loteId && lotes.length > 0) {
      const loteSeleccionado = lotes.find((f) => f.id === loteId);
      if (loteSeleccionado) {
        setLoteData({
          id: loteSeleccionado.id,
          finca_nombre: loteSeleccionado.finca_id || "Desconocida", // Cambiado de finca?.nombre a finca_id
        });
      }
    }
  }, [loteId, lotes]);
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      try {
        ...
        setFormError(errorData.message || "Error al eliminar el lote");
      } catch {
        setFormError(err.message || "Error al eliminar el lote");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative bg-orange-300 rounded-2xl p-6 w-full max-w-md mx-auto transform transition-all duration-300 scale-95 opacity-0 shadow-2xl shadow-orange-950"
        ...
        <div className="space-y-6">
          <p className="text-xl text-orange-800">
            ¿Estás seguro que deseas eliminar el lote de la finca {" "}
            <strong>"{loteData.finca_nombre}"</strong> con <strong>ID: #{loteId}</strong>?
          </p>
