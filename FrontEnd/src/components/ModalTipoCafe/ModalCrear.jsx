...
import { crearTipoCafe } from "../../services/TiposdeCafeService.js";

const ModalCrearTipoCafe = ({
  isOpen,
  fetchTiposCafe,
  onClose,
}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await crearTipoCafe(data);
      fetchTiposCafe();
      onClose();
    } catch (err) {
      console.error("Error al crear tipo de café:", err);
      try {
        const errorData = JSON.parse(err.message);
        setFormError(errorData.message || "Error al crear el tipo de café");
      } catch {
        setFormError(err.message || "Error al crear el tipo de café");
      }
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-orange-300 rounded-2xl p-6 w-full max-w-2xl mx-auto transform transition-all duration-300 scale-95 opacity-0 shadow-2xl shadow-orange-950"
        ...
        <h2 className="text-3xl font-bold text-orange-800 mb-6">Crear Tipo de Café</h2>
...
export default ModalCrearTipoCafe;