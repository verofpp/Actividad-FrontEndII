...
import { eliminarTipoCafe } from "../../services/TiposdeCafeService.js";

const ModalEliminarTipoCafe = ({
  isOpen,
  fetchTiposCafe,
  onClose,
  tipocafeId,
  ...
  const handleEliminar = async () => {
    try {
      await eliminarTipoCafe(tipocafeData);
      await fetchTiposCafe();
      closeError();
    } catch (err) {
      ...
      >
        ...
        <div className="space-y-6">
          <p className="text-xl text-orange-800">
            ¿Estás seguro que deseas eliminar el tipo de café{" "}
            <strong>"{tipocafeData.nombre}"</strong> con <strong>ID: #{tipocafeId}</strong>?
          </p>
...
export default ModalEliminarTipoCafe;
